...
export function ProductCard({ product, variant, image, className }: ProductCardProps) {
  ...
  return (
    <div
      ...
      className={cn(
        'product-card group block cursor-pointer transition-transform duration-300',
    ...
    >
      ...
      <div className="relative aspect-square overflow-hidden bg-muted">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-2">
          ...
          {product.is_featured && <span className="badge-new">Novinka</span>}
        </div>
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex items-baseline gap-2 flex-wrap">
          ...
          {hasDiscount && <span className="price-original">{formatPrice(comparePrice)}</span>}
        </div>
        ...
        {variant && inStock && (
          <Button onClick={handleAddToCart} className="w-full mt-3 md:hidden" size="sm">
            <ShoppingCart className="h-4 w-4 mr-2" />
            Do košíka
          </Button>
        )}
